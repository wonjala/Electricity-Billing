...
<html lang="en">
    ...
    <head>
        ...
        <script>
            function generatePDF(name,number,unit,id,date,bill,amount) {
                var pdf = new jsPDF({
                    orientation: 'p',
                    unit: 'mm',
                    format: 'a5',
                ...
                pdf.text(5, 20, "NEABilling");
                pdf.text(60,30,"Bill Invoice");
                pdf.setFontSize(8);
                pdf.text(10, 50, "User id");
                pdf.text(25, 50, "Invoice no.");
                pdf.text(45, 50, "Full Name ");
                pdf.text(70, 50, "Date");
                pdf.text(90, 50, "Due Date");
                pdf.text(110, 50, "Units");
                pdf.text(130, 50, "Amount");
                ...
                pdf.text(45, 60, name);
                pdf.text(70, 60, date);
                pdf.text(90, 60, number);
                pdf.text(110, 60, unit);
                pdf.text(130, 60, amount);
//                pdf.addPage();
        ...
        <main id="main">
            ...
            <section class="viewbill">
                <div class="container-fluid">
                    ...
                    <h3> Your Bills </h3>
                    <br>
                    <form action="${pageContext.request.contextPath}/PageChange?page=oldbill" method="post">
                        <!--                     <div class="oldbills col-12 d-flex justify-content-between">-->
                        <button class="btn-bills">old bills </button>
...
<!--                                <td data-label="Payment">
                                    <form action="CalcBill?page=paybill" method="post">
                                        <input type="hidden" name="uid" value="<%=request.getParameter("id")%>">
                                        <input type="hidden" name="billid" value="${user.bill.id}">
                                        <input type="hidden" name="amount" value=" <c:choose>
                                                   <c:when test="${user.bill.unit < 20}">${user.bill.unit * 4}</c:when>
                                                   <c:when test="${user.bill.unit < 50}">${user.bill.unit * 7.30}</c:when>
                                                   <c:when test="${user.bill.unit < 150}">${user.bill.unit * 8.60}</c:when>
                                                   <c:when test="${user.bill.unit < 250}">${user.bill.unit * 9.50}</c:when>
                                        ...
                                        <button class="btn-invoice">Make Payment </button></form>
                                </td>-->
                                <td data-label='Action'>
                                    <button class="btn-invoice" onclick="generatePDF('${user.name}','${user.dueDate}','${user.bill.unit}','${user.id}','${user.bill.date}','${user.bill.id}','${user.bill.amount}')">Download</button>
                                </td>
            ...
            <!--                                    <button onclick="Convert_HTML_To_PDF();">Convert HTML to PDF</button>
                                                ...
                                                <div id="contentToPrint">
                                                    ...
                                                    <p>Lorem Ipsum is simply dummy text specimen book.</p>
